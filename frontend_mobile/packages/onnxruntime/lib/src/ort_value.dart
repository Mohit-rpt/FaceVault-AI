...
class OrtValueTensor extends OrtValue {
  ...
  /// Extracts the data as a flat, safely copied Float32List.
  /// Avoids element-by-element boxing and nesting.
  Float32List get flatFloat32List {
    if (_info._tensorElementType != ONNXTensorElementDataType.float) {
      throw Exception('Tensor is not of type float');
    }
    final tensorShapeElementCount = _info._tensorShapeElementCount;
    final dataPtrPtr = calloc<ffi.Pointer<ffi.Float>>();
    final dataPtr = _getTensorMutableData(_ptr, dataPtrPtr);
    final typedList = dataPtr.asTypedList(tensorShapeElementCount);
    // Copy the list so it is safe to use after OrtValue is released
    final copy = Float32List.fromList(typedList);
    calloc.free(dataPtrPtr);
    return copy;
  }
